    ...
    def __init__(
        ...
        **kwargs,
        ...
        if data_freqs_path is not None:
            # Prefer local filesystem when the path points to an existing local file
            # or uses file:// scheme. Only use GCS when path starts with gs:// or
            # when local open fails.
            # data = None
            # try:
            #     if data_freqs_path.startswith("gs://"):
            #         # explicit GCS path
            #         with fs.open(data_freqs_path, "rb") as f:
            #             data = pickle.load(f)
            #     else:
            #         # try local open first
            #         local_path = data_freqs_path
            #         if data_freqs_path.startswith("file://"):
            #             local_path = data_freqs_path[len("file://"):]
            #         if os.path.exists(local_path):
            #             with open(local_path, "rb") as f:
            #                 data = pickle.load(f)
            #         else:
            #             # fallback to GCS (handles remote paths or hydra substitutions)
            #             try:
            #                 with fs.open(data_freqs_path, "rb") as f:
            #                     data = pickle.load(f)
            #             except Exception:
            #                 # re-raise original FileNotFoundError for clarity
            #                 raise
            # except Exception:
            #     # propagate exception so hydra instantiation reports it
            #     raise
